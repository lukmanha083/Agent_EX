defmodule AgentEx.Accounts do
  ...
  alias AgentEx.Accounts.{User, UserNotifier, UserToken}
  ...
  def register_user(attrs) do
    ...
    |> case do
      ...
      {:error, :default_project, _changeset, %{user: user}} ->
        {:error, Ecto.Changeset.add_error(User.registration_changeset(user, %{}), :base, "Failed to create default project")}
    end
  end
  ...
  """
  def change_user_username(%User{} = user, attrs \\ %{}, opts \\ []) do
  ...
  See `AgentEx.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `AgentEx.Accounts.User.password_changeset/3` for a list of supported options.
