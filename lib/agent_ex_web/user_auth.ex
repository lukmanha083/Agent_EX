defmodule AgentExWeb.UserAuth do
  @moduledoc "Plug and LiveView hooks for user authentication."
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      AgentExWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      AgentExWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule AgentExWeb.PageLive do
        use AgentExWeb, :live_view

        on_mount {AgentExWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{AgentExWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_project, _params, _session, socket) do
    if socket.assigns[:current_project] do
      {:cont, socket}
    else
      socket =
        socket
        |> Phoenix.LiveView.put_flash(:info, "Create a project to get started.")
        |> Phoenix.LiveView.redirect(to: ~p"/projects/new")
  ...
  # Redirect to /chat. The :require_project on_mount hook
